...
export default defineComponent({
    setup() {
        ...
        async function postUserLogin(params: any) {
            try {
                loading.value = true
                //   const { data } = await usePostUserLogin(params)
        ...
        return () => {
            return <div class='relative h-lvh '>
               ...
               h-[522px] top-50% -translate-y-50% right-100 p-20'>
                    ...
                    <Form
                        ...
                        model={formState}
                        rules={rules}
                        hideRequiredMark={true} class='mt-12 login-form' layout='vertical'>
                        <Form.Item label="账号" name="name">
                            <Input size="large" placeholder="请输入账号" autocomplete='username'></Input>
                        </Form.Item>
                        <Form.Item label="密码" name="password">
                            <Input.Password size="large" placeholder="请输入密码" autocomplete='current-password'></Input.Password>
                        </Form.Item>
                    </Form>
                    <div class='mb-2'>
                        <Checkbox><span class='text-xs color-tertiary'>记住账号</span></Checkbox>
                    </div>
                    <Button loading={loading.value} onClick={submit}  size="large" class='w-full' type="primary">登录</Button>
                </div>
            </div>
        }
    }
