<template>
  <div v-if="screenStore.isMobile" class="sub-title-wrap pl-6 flex" :class="{ hidden: isHidden }">
    <div
      class="mr-6 whitespace-nowrap"
      v-for="(item, index) in arrTitle"
      @click="handleClick(index)"
    >
      <div
        :class="['item-content', 'font-h5', currentIndex == index ? 'item-active' : 'item-normal']"
      >
        {{ item }}
      </div>
    </div>
  </div>
  <div v-else :class="{ hidden: isHidden }" class="w-full background-colorBgLayout sub-title-wrap">
    <div class="max-w-480 h-16 pl-70 sub-title-wrap flex mx-auto">
      <div class="min-w-136px" v-for="(item, index) in arrTitle" @click="handleClick(index)">
        <div
          :class="[
            'item-content',
            'font-h7',
            currentIndex == index ? 'item-active' : 'item-normal'
          ]"
        >
          {{ item }}
        </div>
